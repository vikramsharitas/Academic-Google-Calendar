...
element = WebDriverWait(driver, 5).until(EC.presence_of_element_located((By.XPATH, '//*[@id="crefli_HC_SSS_STUDENT_CENTER"]/a')))
...
def get_days(row1):#formating days to make it usable as the way to send requests is not the way the given dta is
    a = 0
    if row1[a] == 'M':
        course = 'MO'
    if len(row1) > 1:
        if row1[a] == 'T' and row1[a + 1] != 'h':
            course = 'TU'
    else:
        course = 'TU'
    if row1[a] == 'W':
